...
def quimografia(matriz,iteraciones,n,tao=1,e=1):
    size=14
    mat_graf = matriz[...,:2].sum(-1) 
    
    fig, ax = plt.subplots()
    
    ax.set_xlabel('x',fontsize=size)
    ax.set_ylabel('t',fontsize=size)
    ax.set_title('Población en los canales de velocidad.',fontsize = size)
    
    ax.set_xticks([0,10,20,30,40,50])
    ax.set_xticklabels(np.array([0,10,20,30,40,50])*e)
    
    ax.set_yticks([0,200,400,600,800,1000])
    ax.set_yticklabels(np.array([0,round(200*tao,2),round(400*tao,2),round(600*tao,2),round(800*tao,2),round(1000*tao,2)]))
    
    c = ax.pcolormesh(mat_graf, cmap='hot', vmin=0, vmax=2)
    cb = fig.colorbar(c)

    cb.ax.tick_params(axis='y', which='major', labelsize=size)
    cb.set_label('Cantidad de individuos', fontsize=size)
